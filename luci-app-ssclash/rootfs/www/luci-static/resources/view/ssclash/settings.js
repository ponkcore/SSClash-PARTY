...
async function saveSettings(mode, proxyMode, tunStack, autoDetectLan, autoDetectWan, blockQuic, useTmpfsRules, interfaces) {
    try {
        let detectedLan = '';
        let detectedWan = '';
...
`;
        ...
        ui.addNotification(null, E('p', _('Interface-policy settings saved. Please restart the Clash service for changes to take effect.')), 'info');
        return true;
    } catch (e) {
...
function createAdditionalSettings(blockQuic, useTmpfsRules) {
    const container = E('div', { 'class': 'cbi-section' });
    ...
    render: async function(data) {
        ...
        const saveButton = E('button', {
            ...
            'click': async function() {
                ...
                const savedProxyMode = settings.proxyMode || 'tproxy';
                const savedTunStack = settings.tunStack || 'system';
