...
mkdir -p "$dist"
packages=(
    luci-app-ssclash-4.7.0-r8-openwrt-25.12.5-mediatek-filogic-aarch64_cortex-a53.apk
    luci-app-ssclash-4.7.0-r8-openwrt-25.12.5-x86-64-x86_64.apk
    luci-app-ssclash_4.7.0-r8_x86_64-openwrt-24.10.8-x86-64-x86_64.ipk
)
...
"$generator" "$dist" v4.7.0-party.6 preview "$generated_manifest"
(
    cd "$temporary"
    sha256sum --check --strict ssclash-party-preview-manifest.sha256 >/dev/null
)
grep -Fq 'manifest|1|preview|v4.7.0-party.6|4.7.0-party.6' "$generated_manifest" || \
...
if "$generator" "$bad_sidecar_dist" v4.7.0-party.6 preview \
