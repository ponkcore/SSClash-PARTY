# Router Integration and friendly dashboard
...
- friendly panel publication and hostname.
...
5. Apply the matching dnsmasq/uHTTPd panel publication; restore the previous
   UCI and panel state if this fails.
...
## `panel.router` login flow

When Friendly panel address is enabled, dnsmasq maps the configured hostname
to the router's current LAN interface address. uHTTPd serves a small
host-aware landing page and a static alias for the packaged Zashboard files.
...
The default friendly address uses plain HTTP on the trusted LAN, matching the
ordinary OpenWrt LuCI listener. Browser **Not secure** labeling is expected.
